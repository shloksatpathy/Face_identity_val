...
## Features
- **Live Face Enrollment (`enroll.py`)**: Capture multiple face samples using a webcam and save a robust average embedding.
...
- **Embeddings Storage**: Identities are stored as `.npy` embedding files in the `face/database/` directory.
- **InsightFace Model**: Uses the `buffalo_s` model out of the box with `det_size=(320, 320)`.
